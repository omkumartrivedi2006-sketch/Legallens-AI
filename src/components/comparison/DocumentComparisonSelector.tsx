...
export const DocumentComparisonSelector: React.FC<DocumentComparisonSelectorProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-11 gap-4 items-center">
          ...
          <CardContent className="p-4 sm:p-5 space-y-3">
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-xl bg-slate-50/70 dark:bg-white/[0.04] backdrop-blur-xs border border-slate-200/80 dark:border-white/10 px-3.5 py-2.5 text-xs sm:text-sm font-medium text-slate-900 dark:text-white pr-9 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/40 transition-colors"
              >
                <option value="" disabled className="bg-white dark:bg-[#0D0D0D]">
                  -- Select Base Document --
                </option>
                {readyDocuments.map((doc) => (
                  <option key={doc.id} value={doc.id} className="bg-white dark:bg-[#0D0D0D]">
            ...
            {documentA ? (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50/60 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 text-xs">
        ...
        <div className="lg:col-span-1 flex justify-center">
          <Button
            ...
            className="h-10 w-10 rounded-full p-0 flex items-center justify-center shadow-xs border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-white/[0.04] backdrop-blur-xs hover:border-blue-500/40"
      ...
      {/* Action Bar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 rounded-2xl bg-white/70 dark:bg-white/[0.045] backdrop-blur-md border border-slate-200/80 dark:border-white/10 shadow-xs">
