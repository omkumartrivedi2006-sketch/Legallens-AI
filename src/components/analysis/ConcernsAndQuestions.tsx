        ...
        {concerns.length > 0 ? (
          <div className="space-y-3 pt-1">
            {concerns.map((item, idx) => (
              <div
                key={idx}
                className="p-3.5 rounded-xl border border-amber-200/60 dark:border-amber-900/40 bg-amber-50/30 dark:bg-amber-950/10 space-y-1.5"
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  {item.sourceReference && (
                    <span className="text-[10px] font-mono text-slate-500 bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">
                      {item.sourceReference}
                    </span>
                  )}
                </div>
                ...
                <div className="pt-1.5 text-[11px] text-amber-800 dark:text-amber-300 flex items-start gap-1 font-medium">
          ...
          <p className="text-xs text-slate-400 italic">No unusual or heightened concerns detected.</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
          ...
          {missingInfo.length > 0 ? (
            ...
            <p className="text-xs text-slate-400 italic">No critical omitted sections identified.</p>
          )}
        </div>
          ...
          {lawyerQuestions.length > 0 ? (
            <div className="space-y-2">
              {lawyerQuestions.map((q, idx) => (
                <div
                  key={idx}
                  className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 flex items-start gap-2"
                >
            ...
            <p className="text-xs text-slate-400 italic">No tailored legal questions generated.</p>
          )}
        </div>
      </div>
    </div>
